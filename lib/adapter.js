...
const Adapter = {
  ...
  registerConnection (connection, collections, cb) {
    ...
    let filename =  !_.isEmpty(connection.filename) && connection.filename != ':memory:' ? connection.filename : ('.tmp/' + connection.identity + '.sqlite')
  ...
  query (connectionName, tableName, queryString, args = [ ], cb = args) {
    let cxn = this.connections.get(connectionName)
    console.log('QUERY');
    console.log(Util.toKnexRawQuery(queryString));
    console.log('VALUES');
    console.log(args);
    console.log(Util.castValues(args));
